...
case class PlasmaEffect(width: Int, height: Int) {
  ...
  private def paintPixel(backingArray: Array[Byte], time: Double, x: Int, colX: Double, y: Int, colY: Double) = {
    val v: Double = {
      ...
      val v = wandering + wobbling + floating
  ...
  def sin(angle: Double) = Math.sin(angle)
